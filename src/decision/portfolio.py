...
class Position:
    ...
    # "Why AI Shorted This" (2026-08-21, owner request, same feature as AITrading's own
    # -- see that project's CLAUDE_HISTORY.md 2026-08-21 entry for the full design) --
    # a snapshot of the AI's real buy-time (short-open-time) decision, captured once at
    # the moment of purchase and never regenerated or overwritten by a later
    # re-analysis while the position is held. buy_rr/buy_required_rr are the exact
    # numbers that cleared the gate at that moment. None/"" for any position opened
    # before this field existed -- backfilled once via
    # scripts/backfill_buy_rationale.py rather than left silently blank forever.
    buy_thesis: str = ""
    buy_reasoning: str = ""
    ...
    def unrealized_pnl(self) -> float:
        # Short economics: profit as price falls below entry, not above (inverted
        # from AITrading's long-only market_value - cost_basis -- see
...
class Portfolio:
    ...
    async def initialize(self):
        ...
        # "Why AI Shorted This" snapshot (2026-08-21) -- see Position's own field
        ...
        # "Recent Sell" post-mortem (2026-08-21, same feature as AITrading's own -- see
        # that project's CLAUDE_HISTORY.md 2026-08-21 entry) -- a position's buy-side
    ...
    async def _save_state(self):
        ...
        loop _save_position's own 2026-08-25 fix covers -- ported from AITrading's
        own fix the same day, live incident: "Position update failed: database is
        locked" recurring during real market hours). Same retry-then-warn
        treatment for the identical reason -- this is a real, continuously-
        rewritten snapshot (self-healing on the next successful save), not
        correctness-critical for any single tick, but worth one retry before
        giving up rather than silently losing it with no attempt."""
        if not self._db:
            return
    ...
    async def _save_position(self, position: Position):
        ...
        etc.), called from real sites across order_manager.py/web/app.py/this
        ...
        warning rather than raising (fixed 2026-08-25, ported from AITrading's
        own fix the same day -- live incident: "Position update failed: database
        is locked" recurring during real market hours).
    ...
    async def _remove_position_db(self, ticker: str):
        """Same retry-then-warn treatment as _save_position (2026-08-25, ported
        from AITrading's own fix the same day) -- a failed delete here leaves a
        stale row in the DB for a position that's genuinely closed in memory;
        the position's own in-memory removal from self.positions (done by the
        caller, not here) is what every real decision actually reads, so this
        isn't a live gap, just a DB-tidiness risk that self-heals on the next
        full portfolio sync."""
        if not self._db:
            return
        for attempt in range(2):
