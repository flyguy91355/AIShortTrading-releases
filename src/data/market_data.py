...
class SmaCrossoverInfo:
    ...
    (2026-08-24 design). sma_50/sma_200 are the CURRENT point values (same numbers
    TechnicalIndicators already carries); crossover_date/days_since_crossover describe
    WHEN the relationship most recently flipped into its current state, which
    TechnicalIndicators has no way to answer since it only ever stores a single current
    point, not a series. See get_sma_crossover_info below for how this gets built."""
    ticker: str
    sma_50: float = 0.0
    sma_200: float = 0.0
...
def find_sma_crossover_date(
    ...
    direction="below": SMA50 crossed below SMA200 (a death cross -- this system's
    short-side trend-confirmation signal); direction="above" is the golden-cross
    mirror (AITrading's own long-side equivalent).
...
class MarketDataFetcher:
    ...
    async def get_sma_crossover_info(
        self, ticker: str, direction: str = "below", lookback_days: int = 252, *, hist=None,
    ) -> SmaCrossoverInfo:
        """Stage 2 of the SMA Trend-Confirmation Track (2026-08-24 design) -- fetches 2
        years of daily closes (enough to compute a SMA200 series across a
        `lookback_days` search window, since SMA200 itself needs 200 prior days just
        for its first point) and finds the most recent date the SMA50/SMA200
        relationship flipped into the qualifying state (direction="below": a death
        cross, this system's short-side signal). Fails open (returns a zeroed
        SmaCrossoverInfo) on any fetch error or empty/too-short history, same
        convention as get_long_term_trend/get_technicals above.

        hist= (optional) mirrors get_technicals'/get_historical's own shared-history
        kwarg (2026-08-18, see tests/test_market_data_shared_history.py) -- lets a
        caller that already fetched this ticker's history pass it in directly instead
        of triggering a second real yfinance call, and lets tests supply a synthetic
        DataFrame with no network mocking required."""
        if hist is None:
            try:
