...
class OrderManager:
    ...
    def _buy_snapshot_fields(signal) -> dict:
        """"Why AI Shorted This" snapshot (2026-08-21, same feature as AITrading's
        own -- see that project's CLAUDE_HISTORY.md 2026-08-21 entry), extracted as
        a shared helper 2026-08-25 (ported from AITrading's own fix the same day) --
        the real conviction/signal/thesis/reasoning/R-R numbers behind THIS buy
        decision, captured once from whatever real report/numbers the signal
        actually carries. research_report is None for a handful of non-AI-driven
        signal sources -- falls back to empty/None fields rather than guessing,
        same precedent as every other nullable Position field.

        Extracted from _execute_buy's own immediate FILLED/PARTIAL Position(...)
        call (2026-08-25, ported from AITrading's own live incident the same day --
        owner noticed every held position's buy_thesis etc. were blank, including
        one from days before this session even started) so the two OTHER real
        "pending buy resolved later" reconciliation paths --
        ...
        Position(...) from a stored _pending_stops entry, not from `signal`
        directly -- can compute and stash this dict into that SAME _pending_stops
        entry at registration time (see _execute_buy's PENDING/SUBMITTED branch
        below) and read it back when the real fill eventually resolves. A notional
        buy is submitted as PENDING/SUBMITTED far more often than it resolves
        synchronously as FILLED/PARTIAL, so those two other paths -- not this one
        -- are the ones that actually created nearly every real held position's
        blank snapshot fields."""
    ...
    async def _execute_buy(self, signal) -> Order | None:
        ...
        if result.status in (OrderStatus.FILLED, OrderStatus.PARTIAL):
            ...
            self._pending_stops[signal.ticker] = {
                ...
                # "Why AI Shorted This" snapshot (2026-08-25, ported from AITrading's
                # own fix the same day) -- computed now, from the real signal, since
                # neither real place this pending buy eventually resolves has access
                # to `signal` itself -- only to whatever's stored here. See
                # _buy_snapshot_fields's own docstring for the full incident.
    ...
    async def check_take_profits(self):
        ...
        from the same broker snapshot that caused AITrading's real AIT/CCEP false
        stop-loss trigger, and this function had no freshness guard at all before this
        fix. _tp_pending_confirm is keyed on ticker AND the specific target price so a
