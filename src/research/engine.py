...
def _volatility_scaled_stop_bounds(
    ...
    exceed, exactly as they already are today. Direction-neutral (percentages only)
    -- works identically for AITrading's long-side or this program's short-side
    _clamp_ai_stop_loss."""
    if atr_pct <= 0:
...
class ResearchReport:
    ...
    # 2026-08-24 design) candidate AND Claude judged the SMA death-cross signal a
    # genuine, current, tradeable downtrend -- never assumed True on a missing/failed
    # judgment (see _parse_json_bool's default=False below and AI Data Integrity in
    # CLAUDE.md). Used by _track2_rr_override_applies (web/app.py) to decide whether
    # this candidate can buy despite R/R falling short of its usual gate.
    ...
    # succeed at normally. Added 2026-08-25 (ported from AITrading's own live incident
    # the same day) so a caller iterating many tickers can distinguish "this one ticker
    # had a bad day" from "every remaining call in this run is guaranteed to fail
    # identically" and react accordingly (web/app.py's _sequential_fallback auto-pauses
    # and reports it rather than grinding through the rest of the run for no benefit).
...
- If an SMA TREND SIGNAL section is present below, judge whether it represents a genuine, current downtrend worth shorting — this can justify a short even when risk/reward doesn't fully clear the usual gate. Always answer trend_confirms_entry as false if that section is absent.
...
{{
    ...
    "trend_confirms_entry": <true only if an SMA TREND SIGNAL section is present above AND you judge it a genuine, current, tradeable downtrend — always false if that section is absent>
}}
...
def _build_market_context_section(market_change_pct: float | None) -> str:
    ...
    analysis prompt. Retired the same day a live investigation found this
    account had executed ZERO real trades since going live (2026-08-19) --
    tracing a separate, confirmed prompt bias (recommend_on_deck_retention's own
    injected "real squeeze risk" phrasing, see that function's own docstring)
    led here: this section's own "a rising tide can squeeze a short... be more
    selective and cautious" framing was firing on literally every prompt on
    every day the broad market was up -- which SPY had been, persistently, for
    the entire zero-trade window. Unlike the retention-prompt fix (a clear,
    unintended asymmetry vs. AITrading's neutral wording), this section's
    language was a deliberate, owner-directed design choice from 2026-08-04 --
    but given each stock's own TECHNICAL CONTEXT section already carries its own
    real momentum/trend signal (RSI, SMA, support/resistance), a separate macro
    overlay that leans toward caution more often than confidence (SPY trends up
    more often than down over any extended window) was judged not worth the
    real, demonstrated cost of adding one more "be cautious" lever without a
    symmetric "be aggressive" one. Same change made in AITrading the same day
    (this system's own long-only sibling had the identical section, mirrored
    for long economics)."""
    return ""
...
def _build_sma_trend_section(
    ...
    candidate -- states the real SMA50/SMA200 relationship, how long ago it crossed
    into its current state (or that no crossover was found in the lookback window,
    meaning the stock has held this relationship the whole time), and the company's
    market-cap tier, mirroring _market_cap_tier_label's own "how stale is too stale
    depends on company size" judgment already used by recommend_dip_entry. Returns ""
    when sma_50/sma_200 are non-positive (no usable data), same graceful-omission
    pattern as every other optional prompt section in this file.

    approaching/gap_pct (2026-08-25, ported from AITrading's own fix the same day --
    owner request: "what it needs to look for.. is just about to cross, and has
    already just crossed") -- a second, distinct case alongside the original
    already-crossed one: a candidate whose SMA50/SMA200 gap is small (see
    _sma_near_crossover, web/app.py) but hasn't actually flipped down yet.
    Deliberately a SEPARATE phrasing, not a reuse of the confirmed-cross text with
    different numbers -- stating "a death cross state... is a mechanical signal of
    a confirmed downtrend" about a stock that HASN'T crossed would be factually
    false and could mislead the judgment. The approaching framing asks a narrower
    question (early entry ahead of mechanical confirmation) rather than the
    confirmed framing's "is this still meaningful now that some time has passed"
    question."""
    ...
    if approaching:
        ...
        return (
            ...
            f"SMA50 (${sma_50:.2f}) is still above SMA200 (${sma_200:.2f}), but the gap "
            f"between them is only {gap_text} of the current price — close enough that a "
            f"continuation of the recent trend would flip this into a death cross soon. "
            f"This is NOT yet a confirmed crossover; it's an early read on one that may be "
            f"forming. Company size: {tier_text}.\n"
            f"Judge whether the underlying trend is genuinely weak and durable enough to "
            f"justify entering a short AHEAD of the mechanical confirmation, or whether this "
            f"gap is just as likely to widen back out as it is to close — a larger, more "
            f"stable company's trend is generally more reliable to anticipate this way than "
            f"a small, volatile one's. Set trend_confirms_entry to true only if you genuinely "
    ...
    relationship = "below" if sma_50 < sma_200 else "above"
    ...
    return (
        ...
        f"a death cross state (50-day average below 200-day average) is a mechanical "
        f"signal of a confirmed downtrend. {recency} Company size: {tier_text}.\n"
        f"Judge whether this represents a genuine, CURRENTLY-ACTIONABLE downtrend worth "
        f"shorting even if risk/reward doesn't fully clear the usual gate — weigh how "
        ...
        f"believe this is a real, current, tradeable downtrend — not merely because "
...
def is_account_lockout_error(error: Exception) -> bool:
    ...
    live example, 2026-08-25 incident, first found on AITrading -- this program hit the
    identical wall on its own manual scan the same morning, since both share one
    ANTHROPIC_API_KEY) -- as opposed to an ordinary transient failure (a momentary
    timeout, a single malformed response, a 429 rate limit that a later retry can
    succeed past) that _call_claude_with_retry's own 3-attempt retry loop already
...
"""
    ...
    async def analyze_sell_decision(
        ...
        AITrading's own -- see that project's CLAUDE_HISTORY.md 2026-08-21 entry).
        Fired once per fully-closed SHORT by a periodic job draining Portfolio.
        get_pending_sell_analyses() (this file has no DB access of its own). Judges the
        hold itself -- entry to final cover -- using the real original short thesis,
        every real cover tranche (date/price/reason/pnl), and real price history across
        the window. `tranches` is chronological, one dict per BUY-TO-COVER row for this
        trade_id (date/price/reason/pnl keys); `price_history` is chronological daily
        closes (date/close keys) spanning the hold. Returns None on any failure -- the
        caller leaves the row pending and retries on a later pass, same AI Data
        Integrity principle as every other real trading figure in this codebase never
        guessing.

        Short economics (2026-08-19): a "better cover" means price fell FURTHER than
        where this system actually covered -- the inverse of AITrading's own
        entry-to-exit shape, where a better sell means price rose further."""
        if not self.client:
            return None
        conv_line = f"\nConviction at open: {buy_conviction}/10" if buy_conviction is not None else ""
        rr_line = ""
        if buy_rr is not None and buy_required_rr is not None:
            rr_line = f"\nR/R at open: {buy_rr:.2f} against a required {buy_required_rr:.2f}."
        age_line = (f"\nHeld for approximately {opened_at_days_ago:.0f} day(s)."
                    if opened_at_days_ago is not None else "")
        tranches_str = "\n".join(
            f"- {t['date']}: covered at ${t['price']:.2f} ({t['reason']})"
        ...
        prompt = f"""\
You are writing a post-mortem for a real, already-completed SHORT this system made, to help
improve future cover decisions on this same stock. Judge whether a genuinely better cover
was realistically available given what was known and visible at the time -- not simple
hindsight ("it kept falling later" alone isn't evidence of a mistake if nothing signaled that
at the time).

STOCK: {ticker} — {company_name}
Original short thesis: {buy_thesis}
Original short reasoning: {buy_reasoning}
Entry (short) price: ${entry_price:.2f}{conv_line}{rr_line}{age_line}

Cover tranches (chronological, buy-to-cover):
{tranches_str}

Price history across the hold (chronological daily closes):
{history_str}

Write a genuine, specific assessment: did the cover(s) capture a reasonable outcome given the
setup, or does the price history show a clearly better cover was available (e.g. price
bounced sharply right after a stop-out, or kept falling well past where a take-profit closed
the position)? Be concrete and grounded in the real numbers above -- don't write generic
boilerplate. This is for future reference on this same stock, not a performance score.
...
"""
    ...
    async def analyze_sell_followup(
        ...
        AITrading's own, ported the same day). Fired once, some days after the immediate
        post-mortem, by a periodic job draining Portfolio.
        get_due_sell_analysis_followups() -- revisits the original verdict using real
        price action that has happened SINCE the cover, which wasn't knowable at
        ...
        prompt = f"""\
You previously wrote this post-mortem for a real, completed SHORT on {ticker} — {company_name}:

Verdict: {post_mortem_thesis}
Reasoning: {post_mortem_reasoning}

The position was covered at ${exit_price:.2f}.{age_line} Real price history has accumulated
since then:
{history_str}

With this additional real price action now visible, does your original verdict still hold, or
does it change? Be concrete about what the price did after the cover and what that confirms or
revises about the original cover's timing.

Respond with ONLY a JSON object:
{{"reasoning": "<3-5 sentences, referencing the real post-cover price action above>"}}
"""
        model = self.config.get("research", {}).get("model_dip_entry", "claude-haiku-4-5")
        try:
            response_text = await self._call_claude_with_retry(
...
"""
    ...
    def _build_quick_scan_prompt(
        ...
        adjust its way of thinking?"). In Manual mode the dial is deliberately
        disconnected from the real settings (see apply_risk_tier_to_settings/
        restore_anchors_to_settings), so presenting its posture as the portfolio's
        "current risk tier" would hand Claude a framing that can actively contradict
        whatever the owner has actually hand-set the real gates to -- omitting it
        avoids that, same graceful-omission pattern every other optional section
        here already uses."""
    ...
    async def submit_analysis_batch(
        ...
        sma_contexts (2026-08-27, Track 1/Track 2 merge, ported from AITrading the same
        day, optional, keyed by ticker) folds each qualifying candidate's SMA trend
        context directly into its normal universe-scan prompt -- the same section
        _attempt_near_miss_promotion has always been able to pass via its own
        sma_context parameter, just supplied here instead so a candidate never needs a
        second, separate Claude call later just to add it. Built by the caller
        (web/app.py's chunk-building loop) from the free SMA data quick_screen already
        returns for every survivor, plus a real crossover-date lookup for the (small)
        subset that mechanically qualifies. Missing/omitted-per-ticker entries default
        to None (no section), same as analysis_history_summaries above."""
        if not self.client:
            return None, {}
