"""One-time retroactive backfill for "Why AI Shorted This" (2026-08-21, ported from
AITrading's own feature -- see that project's CLAUDE_HISTORY.md 2026-08-21 entry for
the full design).

Position.buy_thesis/buy_reasoning/etc. are captured automatically for every NEW short
going forward (see OrderManager._execute_buy) at zero extra cost -- this script exists
...
(ResearchEngine.explain_buy_decision, model_dip_entry) that reconstructs a grounded
explanation from the position's real, immutable trade parameters (entry, stop,
targets, fair value if known, conviction if known, the exact R/R math) -- see that
function's own docstring for why this is a faithful reconstruction, not an invented
narrative.

Idempotent: a position that already has buy_thesis is skipped, so re-running this only
ever costs real money for positions still missing it (a previous partial run, or a
position opened since the last run and not yet captured automatically for some reason).
...
async def main():
    ...
    print(f"{len(portfolio.positions)} open position(s), "
          f"{len(candidates)} missing a short rationale.")
