#!/bin/bash
# Cuts a new release on the AIShortTrading-releases distribution repo (path/target
# repo read dynamically from config/settings.yaml's update.releases_repo below --
# this comment names the actual repo this fork now points at; inherited verbatim from
# AITrading, whose own equivalent script points at AITrading-releases instead).
...
# install's dashboard.
#
# 2026-08-12 fix (inherited from AITrading): this script used to ONLY tag whatever
# already happened to be sitting in the releases repo's own git history -- it never
# actually synced fresh code there. Several rounds of real fixes went out this way,
# tagged as if they were included, while the releases repo silently stayed on its
# original bootstrap snapshot the whole time (caught before anyone applied one of the
# stale tags). Now always pushes a fresh mirror of the same allow-listed paths
# is_path_updatable() (src/update/apply.py) permits an apply to touch, before ever
# creating the tag -- so a cut release can no longer silently point at stale code.
